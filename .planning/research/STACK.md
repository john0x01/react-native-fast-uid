# Stack Research — react-native-fast-uid

**Domain:** React Native pure-JSI / C++17 library — synchronous CSPRNG-backed ID generation; new-architecture only; default JS runtime + Reanimated UI worklet runtime
**Researched:** 2026-05-04
**Overall confidence:** MEDIUM (architectural choices HIGH; exact version pins LOW — see "Verification Required" below)

> **Verification required before scaffolding (Phase 1):**
> Network access (WebSearch / WebFetch / npm registry / Bash / Context7) was unavailable during this research pass. Versions below are drawn from training-data knowledge with cutoff January 2026 and must be re-verified by running `npm view <pkg> version` (or equivalent) at scaffold time. **Architectural recommendations (which template, which tool category, why) are HIGH-confidence; exact version pins are LOW-confidence** and flagged inline with `[VERIFY]`.
>
> Locked decisions in `PROJECT.md` Section "Key Decisions" are respected and not re-litigated:
> - Pure JSI install via `jsi::Function::createFromHostFunction` (NOT HostObject, NOT TurboModule codegen)
> - `jsi::String::createFromAscii`
> - `SecRandomCopyBytes` (iOS) / `getrandom(2)` + `/dev/urandom` fallback (Android)
> - New-architecture only
> - Biome (TS) + clang-format (C++)
> - C++17 only (no C++20+)
> - pnpm
...
npx create-react-native-library@latest react-native-fast-uid

# Inside scaffolded repo:
pnpm install

# Reanimated as optional peer for the worklet install path
pnpm add -D react-native-reanimated
...
> **Do NOT pick** "Turbo module" — that scaffolds a codegen spec and forces TurboModule registration, which conflicts with the locked pure-JSI `installOnRuntime` design.
...
**If consumer uses Reanimated (worklet runtime path):**
...
**If consumer is JS-runtime only (no Reanimated):**
...
**If consumer is on RN < 0.76 (old arch):**
...
**If consumer needs Hermes vs JSC:**
...
Run these to convert `[VERIFY]` markers into pinned versions:
...
# Tooling
npm view create-react-native-library version
npm view react-native-builder-bob version
npm view @biomejs/biome version
npm view typescript version
npm view release-it version
npm view @release-it/conventional-changelog version
npm view jest version
npm view mitata version
npm view tinybench version

# Inspect what CRNL actually scaffolds (canonical source of truth)
npx create-react-native-library@latest /tmp/scratch-crnl --type module-new --languages cpp \
  --react-native-version latest --pm pnpm
cat /tmp/scratch-crnl/package.json   # pin from this
cat /tmp/scratch-crnl/.nvmrc 2>/dev/null
cat /tmp/scratch-crnl/Gemfile        # CocoaPods + Ruby pin
cat /tmp/scratch-crnl/android/build.gradle   # NDK + Gradle pin
```

The output of the last block is **the authoritative pin source** — when CRNL and the registry disagree on a version, use what CRNL scaffolds (it's been validated together by the maintainers).
...
> **Network access (WebSearch / WebFetch / npm / Context7 / Bash) was denied during this research pass.** All version pins are training-data-derived (cutoff Jan 2026) and explicitly flagged `[VERIFY]`. The Verification Checklist above must be run before Phase 1 scaffolding to convert pins to ground truth. **Architectural choices (which template, which category of tool, which to avoid) are HIGH confidence** and stable regardless of patch-version drift.
